...
long double run_bandwidth(int size, int stride)
{
    ...
    test_access_memory(num_elements, stride, MAXRUNS * (1 << (int)(log2(MAXBYTES) - log2(size) +1)));
    auto elapsed = std::chrono::high_resolution_clock::now() - start;
    long long microseconds = std::chrono::duration_cast<std::chrono::microseconds>(elapsed).count();
    long double result = (long double)(size / stride);
    result *= MAXRUNS * (1 << (int)(log2(MAXBYTES) - log2(size) +1));
...
long double run_read_file_bandwidth(const char *filename)
{
    ...
    auto elapsed = std::chrono::high_resolution_clock::now() - start;
    long long microseconds = std::chrono::duration_cast<std::chrono::microseconds>(elapsed).count();
    long double result = (long double)size * MAXRUNS * (1 << (int)(log2(MAXBYTES) - log2(size) +1)) / (long double)microseconds;
    return result;
}
...
long double run_latency_ns(int size, int stride)
{
    ...
    long double result = ((long double)microseconds * 1000) / (MAXRUNS * num_elements * (1 << (int)(log2(MAXBYTES) - log2(size) +1)) / (long)stride);
    return result;
}
